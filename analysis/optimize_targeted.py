...
def compute_values(model_name: CheckpointName, algorithm: type[Algorithm]) -> tuple[list[str], list[list[int]], list[list[float]]]:
    ...
    distances, R_values = algorithm.optimize_ray(model_name, 10, M_values)

    return M_values, R_values, distances

def save_results(M_values: list[str], R_values: list[list[int]], distances: list[list[float]], model_name: str, algorithm: str) -> None:
...
def load_results(model_name: str, algorithm: str) -> tuple[list[str], list[list[int]], list[list[float]]]:
...
def plot_results(M_values: list[str], R_values: list[list[int]], distances: list[list[float]], model_name: str, algorithm: str) -> None:
    fig, ax = plt.subplots()
